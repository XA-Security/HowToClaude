# How to Build with Claude — Windows / PC Setup
...
The first time, it asks how to sign in. Choose the Anthropic API key option and paste the `sk-ant-` key from your password manager. You only do this once — it's stored securely on your PC.
...
> [!CAUTION]
> **Where keys live:** Secret keys go in a file called `.env.local` that stays on your PC and is never uploaded. Claude knows this rule — if you ever paste a key in the wrong place, ask it to fix that.
...
- [ ] Scoop, Node + Git, and Claude Code installed
